"""
CLI for Stable Signature watermarking method.
"""
...
def create_parser() -> argparse.ArgumentParser:
    """Create argument parser for Stable Signature CLI."""
    parser = argparse.ArgumentParser(
        description="Stable Signature Watermarking CLI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Embed watermark in single image
  stable-signature embed input.jpg output.jpg --message "my_watermark"
  
  # Detect watermark in image
  stable-signature detect watermarked.jpg
  
  # Process folder of images
  stable-signature embed-folder input_dir/ output_dir/ --message "batch_watermark"
        """
    )
    ...
    embed_parser.add_argument("--message", default="stable_signature_48b", help="Watermark message")
    embed_parser.add_argument("--model-path", type=Path, help="Path to decoder model")
    ...
    detect_parser.add_argument("--model-path", type=Path, help="Path to decoder model")
    ...
    embed_folder_parser.add_argument("--message", default="stable_signature_48b", help="Watermark message")
    embed_folder_parser.add_argument("--model-path", type=Path, help="Path to decoder model")
...
def embed_single(args) -> int:
    ...
    config = {}
    if args.model_path:
        config['decoder_path'] = str(args.model_path)
    
    if not method.initialize(config):
        logger.error("Failed to initialize Stable Signature method")
        return 1
...
def embed_folder(args) -> int:
    ...
    images = load_images_from_folder(args.input_dir, max_images=args.max_images)
    ...
    for filename, image in images:
        ...
        # Save result
        output_path = args.output_dir / f"wm_{filename}"
        if save_image(watermarked_image, output_path):
            success_count += 1
        else:
            logger.warning(f"Failed to save {filename}")
    
    logger.info(f"✅ Successfully processed {success_count}/{len(images)} images")
